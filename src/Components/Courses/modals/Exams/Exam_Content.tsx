...
export default function Exam_Content({question,userAnswers,setUserAnswers}:{question:Question_Interface,userAnswers:UserAnswer_Interface[],setUserAnswers:Dispatch<SetStateAction<UserAnswer_Interface[]>>}) {
    ...
    return (
    <article className="bg-white  min-w-[100%] h-fit p-1 md:p-3 rounded-2xl ">
        <div className=" p-1 md:p-2 rounded-2xl h-[100%] w-[100%]">
            {/*Question Number*/}
            <span className="text-lg md:text-2xl font-medium ">{question?.id}.</span>
            {/*Question*/}
            <p className="text-lg md:text-xl font-medium my-2">{question?.question}</p>
            {/*List Of Choes*/}
            <ul className="flex flex-col justify-start items-start gap-1">
                {
                    question?.options?.map(chose=>
                        <li key={chose} className="text-xl font-medium flex w-full bg-white shadow-lg rounded-lg   has-[input:checked]:bg-[#5d7aff] has-[input:checked]:text-white justify-start items-center gap-2">
                            <div className="relative w-[25%] md:w-[10%] border-r-2 border-gray-200 py-1 h-full flex justify-center items-center">
                            ...
                            <label className="cursor-pointer p-3  w-full h-full" htmlFor={`${chose}-${question?.id}-${question?.question}`}>
                                {chose}
                            </label>
                        </li>
                    )
